...
async def monitor_import(
    ...
    if content.startswith("[") or content.startswith("{"):
        try:
            ...
            for item in payload:
                ...
                if scheduler and monitor.is_active:
                    logger.debug("monitor_import: scheduler=%s monitor_id=%s interval=%s", bool(scheduler), monitor.id, interval_sec)
                    scheduler.add_monitor(monitor.id, interval_sec)
                created += 1
            await db.commit()
        except Exception as exc:
        ...
        for line in content.splitlines():
            ...
            await db.flush()
            if scheduler and monitor.is_active:
                logger.debug("monitor_import (plain): scheduler=%s monitor_id=%s interval=%s", bool(scheduler), monitor.id, interval_sec)
                scheduler.add_monitor(monitor.id, interval_sec)
            created += 1
        await db.commit()
...
async def api_logs(
    ...
    system = list(reversed(log_manager.get_system_logs()))[:20]
...
async def api_bot_start(
    ...
    if not user.telegram_bot_token:
        return JSONResponse({"ok": False, "message": "Токен не задан"}, status_code=400)
...
async def api_bot_stop(
    ...
    return JSONResponse({"ok": True, "message": "Бот остановлен"})
...
async def api_bot_test(
    ...
    """Send a test Telegram message to the user's chat."""
    if not user.telegram_bot_token or not user.telegram_chat_id:
        return JSONResponse({"ok": False, "message": "Бот не настроен"}, status_code=400)
    ...
    try:
        ...
        return JSONResponse({"ok": True, "message": "Тест отправлен"})
    except Exception as exc:
...
@router.post("/api/monitors/{monitor_id}/trigger")
async def api_monitor_trigger(
    ...
    if scheduler and scheduler.trigger_now(monitor_id):
        return JSONResponse({"ok": True, "message": "Проверка запущена"})
    return JSONResponse({"ok": False, "message": "Планировщик недоступен"}, status_code=503)


# ---------------------------------------------------------------------------
# /settings/start-bot and /settings/stop-bot  (used by test_bot_startup_fix)
# ---------------------------------------------------------------------------
...
async def settings_start_bot(
    ...
    if web_deps.is_bot_running(user.telegram_bot_token):
        return JSONResponse({"ok": True, "message": "Бот уже запущен"})
...
# ---------------------------------------------------------------------------
# import_monitors – alias for test_import_export.py compatibility
# ---------------------------------------------------------------------------
...
async def import_monitors(
    ...
    """
    Direct-call alias used by tests.  Wraps monitor_import logic without
    requiring an HTTP form submission.
    """
