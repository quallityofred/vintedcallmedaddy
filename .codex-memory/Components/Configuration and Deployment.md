...
- 2026-06-05: Supabase Session pooler production logs showed `EMAXCONNSESSION` because the pooler is capped at 15 clients. Backend defaults are now session-pooler safe: `DB_USE_NULL_POOL=false`, `DB_POOL_SIZE=3`, `DB_MAX_OVERFLOW=0`, `DB_POOL_TIMEOUT=10`, and `DB_POOL_RECYCLE_SECONDS=120`. Keep asyncpg statement cache disabled and `pool_pre_ping=true`. Direct Supabase connection remains unsuitable unless IPv4 direct connectivity is enabled for the project.
- 2026-06-05: `/api/health` exposes safe pool diagnostics (`database_pool_mode`, `database_pool_size`, `database_max_overflow`, `database_pool_timeout_seconds`, `database_pool_recycle_seconds`) without printing DB URLs. `EMAXCONNSESSION`/session-pooler max-client errors are classified as DB capacity outages and return controlled `503` responses.
