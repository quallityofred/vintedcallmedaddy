...
- 2026-06-08: Delta knowledge now combines domain-scoped `SeenItem` rows with monitor-level `FoundItem` history. Existing Found history is never requeued; missing Seen mappings are inserted idempotently. Same-domain Found history may stop that domain at the newest-first boundary, while Found history from another domain suppresses only the duplicate item and does not stop later candidates. All selected domains remain checked.
- 2026-06-08: Fixed baseline and full-cycle router calls that had lost the required `monitor` argument and public helper imports. This could prevent an intended baseline from being written before resume/check-now.
