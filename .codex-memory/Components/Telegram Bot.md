...
- 2026-06-11: `issue/fixed` Added suspect brand-filter backlog quarantine support at `POST /api/v1/maintenance/pending-notifications/ack-suspect-brand-filter-backlog`. It is dry-run by default, admin/API-CSRF protected, and live mode requires typed confirmations. Eligibility is conservative: only pending rows for brand-filter monitors that lack matching brand ID/title evidence are acknowledged no-notify; verified brand rows stay pending for normal delivery. The endpoint sends no Telegram and deletes no history.
